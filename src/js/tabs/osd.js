...
OSD.loadDisplayFields = function() {
 ...
 // All display fields, from every version, do not remove elements, only add!
    OSD.ALL_DISPLAY_FIELDS = {
        ...
        SYS_DISTANCE: {
            name: 'SYS_DISTANCE',
            text: 'osdTextElementSysDistance777',
                ...
                .then(info => {
                    ...
                    for (let i = 0; i < OSD.constants.VIDEO_TYPES.length; i++) {
                        ...
                        const $checkbox = $('<label/>')
                            .append($(`<input class="radioVideo"  name="video_system" type="radio"/>${videoFormatOptionText}</label>`)
                    ...
                    for (let i = 0; i < OSD.constants.UNIT_TYPES.length; i++) {
                        ...
                        const $checkbox = $('<label/>')
                            .append($(`<input class="radioVideo" name="unit_mode" type="radio"/>${setupUnitOptionText}</label>`)
                    ...
                    for (const field of OSD.data.displayItems) {
                        ...
                        for (let osd_profile = 0; osd_profile < OSD.getNumberOfProfiles(); osd_profile++) {
                            $field.append(
                                    $(`<input type="checkbox" class="custom-checkbox" name="${field.name}"></input>`)
                                        .data('field', field)
